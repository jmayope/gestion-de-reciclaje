        ...
        actions: [
          if (hasWastes)
            TextButton(
              ...
              child: Text(_allSelected ? 'Ninguno' : 'Todos'),
            ),
        ],
      ),
      body: SafeArea(
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _SelectableWasteCard(
                          waste: waste,
                          selected: _selected.contains(waste.id),
                          onTap: () => _toggle(waste.id),
                        );
                      },
                    ),
                  ),
          ...
          ? SafeArea(
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border(
                    top: BorderSide(color: isDark ? AppColors.line700 : AppColors.line200),
                  ),
                ),
                child: Row(
                  children: [
                    Expanded(
                      child: Text(
                        _selected.isEmpty
                            ...
                            : '${_selected.length} residuo(s) seleccionado(s)',
                        style: theme.textTheme.bodyMedium,
                      ),
                    ),
                    const SizedBox(width: 12),
                    ElevatedButton.icon(
                      onPressed: _selected.isEmpty ? null : confirmPublish,
...
class _SelectableWasteCard extends StatelessWidget {
  final _GroupedWaste waste;
  final bool selected;
  final VoidCallback onTap;

  const _SelectableWasteCard({
    required this.waste,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(waste.id, style: theme.textTheme.labelSmall),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Icon(Icons.scale_outlined, size: 14, color: AppColors.ink400),
                      const SizedBox(width: 5),
                      Text('${waste.quantityTon} ton', style: theme.textTheme.bodyMedium),
                    ],
                  ),
...
class PublishConfirmSheet extends StatelessWidget {
  final List<_GroupedWaste> wastes;
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;
    final totalTon = wastes.fold<double>(0, (sum, w) => sum + w.quantityTon);
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(w.type, style: theme.textTheme.titleMedium),
                            Text(w.id, style: theme.textTheme.labelSmall),
                          ],
                        ),
                      ),
                      Text('${w.quantityTon} ton', style: theme.textTheme.bodyMedium),
                    ],
                  );
                },
              ),
